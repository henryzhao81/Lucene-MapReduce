...
ps -ef | grep java | grep xdiFilter
if [ $? == 0 ]; then
...
HADOOP_USER_NAME=${APP_USER} hadoop jar ${XDI_HOME}/lib/xdi-audience-grid-${XDI_VERSION}-with-dependencies.jar com.openx.audience.xdi.xdiFilter.XdiFilterDriver

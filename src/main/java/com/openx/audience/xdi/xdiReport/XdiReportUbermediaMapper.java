...
public class XdiReportUbermediaMapper extends Mapper<Object, Text, Text, Text> {
  ...
  public void map(Object key, Text value, Context context) throws IOException, InterruptedException {   
    ...
    if(words.length > 1) {
      ...
      if(result[1] != null && result[1].length() > 0 && !result[1].equals("\\N")) {
        ...
        second2.set(result[1] + "&ubermedia");
      }
      if(result[0] != null && result[0].length() > 0 && !result[0].equals("\\N")) {
        second.set(result[0] + "&ubermedia"); //Key become Value+Vendor
        first2.set(result[0]);
